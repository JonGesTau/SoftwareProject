...
ChessWidget* createChessNewGameButton(SDL_Renderer* windowRender, SDL_Rect* location,
    ...
    ChessNewGameButton* data = (ChessNewGameButton*) malloc(sizeof(ChessNewGameButton));
    ...
    data->isActive = false;
    res->destroyWidget = destroyChessNewGameButton;
    res->drawWidget = drawChessNewGameButton;
    res->handleEvent = handleChessNewGameButtonEvenet;
    res->data = data;
    return res;
}

//You need this function in order to destroy all data Associate with a button:
void destroyChessNewGameButton(ChessWidget* src) {
    if (src == NULL ) {
        return;
    }
    ChessNewGameButton* castData = (ChessNewGameButton*) src->data;
